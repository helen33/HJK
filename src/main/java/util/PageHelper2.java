...
public class PageHelper2 {
	...
	public StringBuffer getPageUrl(int currentPage, int totalNum) {
		...
		if(currentBlock > 1){
			PageUrl.append("<span><a href=\"esmain.do?es_page=/qnaForm.do?page=" + (currentPage-1) + "\"><이전></a>&nbsp;&nbsp;");
		}else{
			PageUrl.append("<span>");
		}
		...
		for(int i = startPage ; i <= lastPage ; i++) {
			if(i == currentPage){
				...
				PageUrl.append(".&nbsp;<a href=\"esmain.do?es_page=/qnaForm.do?page=" +i + "\" class=\"page\">" + i + "</a>&nbsp;");
			}		
		}

		if(currentBlock <= totalBlock){
			PageUrl.append(".&nbsp;&nbsp;<a href=\"esmain.do?es_page=/qnaForm.do?page=" + (currentPage+1) + "\"><다음></a></span>");
		}else{
			PageUrl.append(".</span>");
		}
